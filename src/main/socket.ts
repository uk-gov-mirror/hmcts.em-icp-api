...
const socket = (server: Server) => {
  ...
  const pub = nodeRedis.createClient(config.redis.port, config.redis.host, redisOptions);
  const sub = nodeRedis.createClient(config.redis.port, config.redis.host, redisOptions);
